	...
	, _NCA(new QNetworkConfigurationManager(this))
{
	...
	// startup if webserver is running & localAddress is found
...
const QString SSDPHandler::buildDesc()
{
	...
	return SSDP_DESCRIPTION.arg(getBaseAddress(), QString("Hyperion (%2)").arg(_localAddress), QString(HYPERION_VERSION), Stats::getInstance()->getID());
}
