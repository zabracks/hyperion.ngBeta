	...
	, _image_stream_timeout(0)
{
	...
	// from Plugins
	connect(_plugins, &Plugins::pluginAction, this, &JsonProcessor::doPluginAction);
	// to Plugins
	connect(this, &JsonProcessor::pluginAction, _plugins, &Plugins::doPluginAction);
...
void JsonProcessor::handleConfigSetCommand(const QJsonObject& message, const QString &command, const int tan)
{
	if (message.contains("config"))
	{
		...
		try
		{
			...
			{
				QJsonFactory::writeJson(_hyperion->getConfigFilePath(), hyperionConfigJsonObj);
			}
...
void JsonProcessor::handleManagementCommand(const QJsonObject& message, const QString &command, const int tan)
{
...
void JsonProcessor::handlePluginCommand(const QJsonObject& message, const QString &command, const int tan)
{
	const QString& subc = message["subcommand"].toString();
...
void JsonProcessor::doPluginAction(PluginAction action, QString id, bool success, PluginDefinition def)
{
...
void JsonProcessor::sendSuccessDataReply(const QJsonObject &data, const QString &command, const int &tan)
{
	QJsonObject reply;
	reply["success"] = true;
	reply["command"] = command;
